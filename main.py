...
def merge_classification(
    ...
    Parameters:
        base (QueryClassification): The existing classification state.
        update (QueryClassification): The new classification update.
...
def deduplicate_sources(response: str) -> str:
    ...
    Returns:
        str: The response text with deduplicated and updated source citations.
    """
...
def _collapse_duplicate_inline_citations(text: str) -> str:
    ...
    Parameters:
        text (str): Output text containing citation brackets.

    Returns:
        str: Text with redundant successive references to the same source collapsed.
    """
    pattern = re.compile(
...
def _source_reference(doc: Document) -> str:
    ...
    Parameters:
        doc (Document): The document to generate a reference for.

    Returns:
        str: A human-readable reference string.
    """
...
def _strip_model_sources_section(text: str) -> str:
    ...
    Returns:
        str: The text with the sources section removed.
    """
...
def _source_key_and_label(doc: Document) -> tuple:
    ...
    Parameters:
        doc (Document): The document to extract the key and label from.

    Returns:
        tuple: A tuple containing (deduplication_key_tuple, string_label).
    """
...
def _extract_citation_numbers(text: str) -> List[int]:
    ...
    Parameters:
        text (str): The text to extract citation numbers from.

    Returns:
        List[int]: A list of integers representing the cited source numbers.
    """
    numbers: List[int] = []
...
def _rewrite_citation_blocks(text: str, old_to_new: dict) -> str:
    ...
    Parameters:
        text (str): Text containing citation blocks.
        old_to_new (dict): Dictionary mapping old source indices to new source indices.

    Returns:
        str: Text with modernized and mapped inline citation blocks.
    """
...
def finalize_response_with_sources(answer: str, docs: List[Document]) -> str:
    ...
    Parameters:
        ...
        docs (List[Document]): The retrieved documents used as context.

    Returns:
        str: Formatted final answer text with a valid, clean Sources appendix.
    """
...
def format_history(history: deque[dict]) -> str:
    ...
    Parameters:
        history (deque[dict]): The conversation history containing user and assistant turns.

    Returns:
        str: A formatted multiline string block of the history.
    """
    if not history:
...
def get_missing_fields(clf: QueryClassification) -> list[str]:
    ...
    Parameters:
        clf (QueryClassification): The current query classification.
...
def ask_followup(question: str, missing: list[str]) -> str:
    ...
    Parameters:
        question (str): The accumulated queries/messages from the user so far.
        missing (list[str]): List of plain-text descriptions of missing info.

    Returns:
        str: The synthesized natural text follow-up question.
    """
...
def classify_query(
    ...
    Parameters:
        question (str): The most recent user input block.
        current (Optional[QueryClassification]): The existing extracted facts.

    Returns:
        QueryClassification: Structured extraction merging new facts with previous ones.
    """
...
def build_extraction_block(clf: QueryClassification) -> str:
    ...
    Parameters:
        clf (QueryClassification): The accumulated state of established facts.
...
def build_constraint_block(clf: QueryClassification) -> str:
    ...
    Returns:
        str: A multiline string of instructions for the LLM to adhere to constraints.
    """
    constraints = []
...
def rewrite_query(
    ...
    Parameters:
        question (str): The raw text asked to the system.
        clf (Optional[QueryClassification]): Supplied classification of the query, if it exists.

    Returns:
        str: A clean, direct retrieval query for the embedding model.
    """
    if clf is None:
...
def ask(question: str) -> str:
    ...
    Returns:
        str: The final output answering the user or following up for instructions.
    """
...
def ask_and_print(question: str) -> None:
    """
    A helper to process an initial question and print the answer wrapped with borders.
    
    Parameters:
        question (str): The initial query for the pipeline to start on.
